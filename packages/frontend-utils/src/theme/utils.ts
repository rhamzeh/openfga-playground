...
import { OpenFgaDslThemeToken, OpenFgaDslThemeTokenType, type OpenFgaThemeConfiguration } from './types.js';

/**
 * Maps each concrete Monaco token to its semantic token type category.
 * Used by buildMonacoTheme to generate Monaco theme rules.
 */
const tokenTypeMap: Record<OpenFgaDslThemeToken, OpenFgaDslThemeTokenType> = {
  ...
  [OpenFgaDslThemeToken.KEYWORD_MODULE]: OpenFgaDslThemeTokenType.KEYWORD,
  [OpenFgaDslThemeToken.KEYWORD_EXTEND]: OpenFgaDslThemeTokenType.KEYWORD,
  ...
  [OpenFgaDslThemeToken.VALUE_MODULE]: OpenFgaDslThemeTokenType.TYPE,
...
/**
 * Resolves the foreground color and optional font style for a single Monaco token
 * given a theme configuration. Respects rawColorOverrides and rawStylesOverrides.
 */
export function getThemeTokenStyle(token: OpenFgaDslThemeToken, themeConfig: OpenFgaThemeConfiguration): TokenStyle {
  const tokenType = tokenTypeMap[token] ?? OpenFgaDslThemeTokenType.DEFAULT;
  return {
    foreground:
      themeConfig.rawColorOverrides?.[token] ??
      themeConfig.colors[tokenType],
    fontStyle:
      themeConfig.rawStylesOverrides?.[token] ??
      themeConfig.styles?.[tokenType],
  };
}
