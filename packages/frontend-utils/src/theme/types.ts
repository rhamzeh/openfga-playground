...
/**
 * Semantic token type categories. Each maps a high-level concept (TYPE, RELATION, etc.)
 * to the concrete Monaco token strings via the token map in utils.ts.
 */
...
/**
 * Concrete Monaco token strings emitted by the Monarch tokenizer.
 * These appear in theme rules as `token.dsl.openfga`.
 */
